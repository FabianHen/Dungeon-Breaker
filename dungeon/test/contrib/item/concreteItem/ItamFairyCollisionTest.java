...
class ItemFairyCollisionTest {
  ...
  void setUp() {
    try (MockedStatic<Game> mockedGame = mockStatic(Game.class);
         ...
         MockedConstruction<CollideComponent> mockedComponent = mockConstruction(CollideComponent.class,
           (mock, context) -> {
             this.onCollideHandler = (TriConsumer<Entity, Entity, Direction>) context.arguments().get(0);
           })) {

      mockedGame.when(() -> Game.tileAt((Point) any())).thenReturn(Optional.of(mock(FloorTile.class)));
      mockedBuilder.when(() -> WorldItemBuilder.buildWorldItemSimpleInteraction(any(), any())).thenReturn(mock(Entity.class));
  ...
  /**
   * Prüft den perfekten Ablauf: Der Spieler berührt die Fee, seine Lebenspunkte
   * werden auf das Maximum gesetzt und die Fee verschwindet.
   */
  @Test
  ...
  /**
   * Stellt sicher, dass die Fee auch dann aus dem Spiel entfernt wird, wenn der Spieler
   * (z. B. durch einen Bug) gar keine Lebenspunkte-Komponente besitzt.
   */
  @Test
  ...
  /**
   * Verifiziert, dass Monster oder NPCs die Fee nicht aus Versehen einsammeln können
   * und sie in diesem Fall einfach liegen bleibt.
   */
  @Test
  ...
  /**
   * Prüft, dass das Spiel nicht abstürzt, wenn im Moment der Kollision aus irgendeinem
   * Grund kein aktiver Spieler im System registriert ist.
   */
  @Test
  ...
  /**
   * Bestätigt, dass fehlerhafte Kollisionsaufrufe der Engine (bei denen der Kollisionspartner
   * null ist) sofort zu einer erwarteten Exception führen, statt stillschweigend Fehler zu verursachen.
   */
  @Test
