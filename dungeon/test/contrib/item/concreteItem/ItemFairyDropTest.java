...
class ItemFairyDropTest {
  ...
  /**
   * Verifiziert den Erfolgsfall: Wenn die Zielposition ein begehbarer Boden ist,
   * wird das Item korrekt als Entität erstellt und in die Spielwelt eingefügt.
   */
  @Test
  void testDropG1_PositionOnFloorTile() {
    ...
    try (MockedStatic<Game> mockedGame = mockStatic(Game.class);
      ...
      mockedBuilder.when(() ->
        WorldItemBuilder.buildWorldItemSimpleInteraction(any(Item.class), any(Point.class))
      ...
      assertEquals(localMockEntity, result.get(), "Das zurückgegebene Item entspricht nicht der erstellten Entität.");
  ...
  /**
   * Prüft, dass auf ungültigen Feldern (wie Wänden) der Drop abgebrochen wird
   * und keine Entität im Spiel landet.
   */
  @Test
  void testDropG2_PositionOnOtherTile() {
    ...
    try (MockedStatic<Game> mockedGame = mockStatic(Game.class);
      ...
      assertEquals(true, result.isEmpty(), "Das Optional sollte leer sein, wenn es kein FloorTile ist.");

      // Assert: Es wird keine Entität dem Spiel hinzugefügt
      mockedGame.verify(() -> Game.add((Entity) any()), never());
      mockedBuilder.verify(() -> WorldItemBuilder.buildWorldItemSimpleInteraction(any(), any()), never());
    }
  }

  /**
   * Stellt sicher, dass Positionen außerhalb der Karte (wo gar keine Felder existieren)
   * ebenfalls sicher ignoriert werden.
   */
  @Test
  void testDropG3_PositionOutsideMap() {
    ...
    try (MockedStatic<Game> mockedGame = mockStatic(Game.class);
      ...
      // Assert:
      assertEquals(true, result.isEmpty(), "Das Optional sollte leer sein, wenn die Position außerhalb der Map liegt (null).");
  ...
  /**
   * Kontrolliert, dass eine komplett fehlende Positionsangabe (null) nicht heimlich verschluckt wird,
   * sondern die zugehörige Exception sauber nach oben durchgereicht wird.
   */
  @Test
  void testDropU1_NullReferenceForPosition() {
    // Arrange
    try (MockedStatic<Game> mockedGame = mockStatic(Game.class)) {
      mockedGame.when(() -> Game.tileAt((Point) null)).thenThrow(new NullPointerException("Position cannot be null"));
